...
import { animationManifest, mapAsset, shelterAsset, sourceAnimationEntries } from './manifest.mjs';
...
const missingCandidates = [
  ...sourceAnimationEntries.map(({ source }) => source),
  shelterAsset.source,
...
const missingRuntime = [
  ...sourceAnimationEntries.map(({ url }) => `public${url}`),
  shelterAsset.output,
  mapAsset.output,
].filter((file) => !existsSync(file));
...
async function renderAnimationHtml() {
  ...
  const shelterRows = await frameBounds(shelterAsset.output, 4);
  const shelter = {
    dataUrl: `data:image/png;base64,${(await readFile(shelterAsset.output)).toString('base64')}`,
    bounds: shelterRows,
    cssSize: 256 * (100 / shelterAsset.opaqueHeightPx) * VIEWPORT_FIT,
    outlineAt390: await measureOutlineAt390(shelterAsset.output, {
      frameCount: 4,
      logicalOpaqueHeight: 100,
      sourceOpaqueHeight: shelterAsset.opaqueHeightPx,
    }),
  };
  const encodedAnimations = JSON.stringify(animations).replaceAll('<', '\\u003c');
  const encodedShelter = JSON.stringify(shelter).replaceAll('<', '\\u003c');
  ...
  *{box-sizing:border-box}html,body{width:390px;max-width:390px;margin:0;overflow-x:hidden}body{font:12px system-ui,sans-serif;background:#334155;color:white}header{position:sticky;top:0;z-index:3;width:390px;padding:10px 12px;background:#1e293b;border-bottom:1px solid #64748b;font-size:11px;line-height:1.35}main{width:390px;padding:8px;display:grid;gap:10px}section{padding:8px;background:#1e293b;border:1px solid #475569;border-radius:10px}section>h1{margin:0 0 3px;font-size:14px}section>.section-note{margin:0 0 8px;color:#cbd5e1;font-size:10px;line-height:1.35}article{margin-top:8px;padding:7px;background:#0f172a;border-radius:8px;overflow:hidden}article>h2{margin:0 0 5px;font-size:11px;overflow-wrap:anywhere}.stage{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px;padding:5px;background:repeating-conic-gradient(#e2e8f0 0 25%,#94a3b8 0 50%) 50%/16px 16px}.frame-card{min-width:0;padding:3px;background:rgba(15,23,42,.88);border-radius:5px}.frame-visual{position:relative;margin:0 auto}.frame-visual canvas{display:block;width:100%;height:100%}.frame-meta{margin-top:3px;color:#e2e8f0;font:8px/1.25 ui-monospace,SFMono-Regular,monospace;overflow-wrap:anywhere}.sheet-meta{margin-top:4px;color:#cbd5e1;font-size:9px}.semantic-stage{display:flex;flex-wrap:wrap;align-items:end;gap:7px;padding:7px;background:#f1eadb;border-radius:7px}.semantic-stage .frame-card{width:108px;color:#1f2937;background:#fffaf0}.semantic-stage .frame-meta{color:#334155}.socket-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px;margin-top:5px}.socket-chip{padding:4px;border:1px solid #eab308;border-radius:5px;color:#fef08a;font-size:9px}.label-demo{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:8px;background:#e7efd8;border-radius:7px}.enemy-tag{display:grid;justify-items:center;gap:2px}.hp-demo{width:94px;height:6px;padding:1px;background:#30271f;border-radius:3px}.hp-demo>i{display:block;height:4px;border-radius:2px}.enemy-label{padding:2px 5px;border-radius:4px;color:white;background:rgba(47,37,31,.86);font-size:10px;font-weight:800;text-shadow:-1px 0 #30271f,0 1px #30271f,1px 0 #30271f,0 -1px #30271f}.damage-demo{display:flex;justify-content:space-around;align-items:end;min-height:52px;margin-top:8px;padding:8px;background:#3f4d35;border-radius:7px}.damage-demo span{font-weight:900;text-shadow:0 1px 2px #30271f}.shelter-local-hp{display:grid;justify-items:center;gap:3px;margin:8px 0;padding:8px;background:#e7efd8;color:#30271f;border-radius:7px}.shelter-local-hp .bar{width:96px;height:8px;padding:1px;background:#30271f;border-radius:4px}.shelter-local-hp .bar i{display:block;width:62%;height:6px;background:#f2b84b;border-radius:3px}.dock-demo{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}.skill-entry{min-width:0;display:grid;grid-template-columns:22px 1fr;grid-template-rows:1fr 1fr;align-items:center;padding:5px;border:2px solid #d7a62d;border-radius:9px;color:#574b3f;background:#fffaf0;font-size:10px;font-weight:800}.skill-entry svg{grid-row:1/3;width:20px;height:20px;color:#6b5b4a}.skill-entry small{font-size:9px}.legend{color:#fde68a}
  ...
  <main id="root">
    ...
    <section data-review-section="tail-swipe-event-frame"><h1>후추 꼬리치기 판정 프레임</h1><p class="section-note">6 frames · eventFrame 3 · 360도 판정 시점</p><div data-section-body></div></section>
    ...
    <section data-review-section="shelter-hp-and-skill-dock"><h1>보호소·로컬 HP·기술 도크</h1><p class="section-note">네 보호소 상태와 보호소 아래 HP, 세 단색 기술 entry</p><div data-section-body></div></section>
  </main>
  <script>const animations=${encodedAnimations};const shelter=${encodedShelter};
  ...
  {const article=document.createElement('article');article.dataset.sheet='shelter-states';article.dataset.kind='shelter';article.dataset.frameCount='4';const title=document.createElement('h2');title.textContent='shelter-states';const stage=document.createElement('div');stage.className='stage';appendFrames(shelter,stage,'shelter',['healthy','damaged','critical','failed']);const meta=document.createElement('div');meta.className='sheet-meta';meta.textContent='4 states · outline '+shelter.outlineAt390.toFixed(2)+'px';article.append(title,stage,meta);const localHp=document.createElement('div');localHp.className='shelter-local-hp';localHp.dataset.shelterLocalHp='true';localHp.innerHTML='<strong>보호소</strong><div class="bar"><i></i></div><span>620 / 1000</span>';const dock=document.createElement('div');dock.className='dock-demo';for(const [name,cost,pathData] of [['꼬리치기','15','M4 12h16M20 4v16'],['아쿠아빔','25','M5 20c5-12 9-12 14 0'],['안전신문고','40','M8 4h14v20H8z']]){const entry=document.createElement('div');entry.className='skill-entry';entry.dataset.skillEntry=name;entry.innerHTML='<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="3"><path d="'+pathData+'"/></svg><span>'+name+'</span><small>간식 '+cost+'</small>';dock.append(entry)}bodyFor('shelter-hp-and-skill-dock').append(article,localHp,dock)}
  <\/script></body></html>`;
  await writeFile(`${outputDir}/sprite-animation-review.html`, html);
}

async function renderMapOverlay() {
  const background = await sharp(mapAsset.output).resize(390, 693, { fit: 'fill' }).png().toBuffer();
  const shelter = await sharp(shelterAsset.output)
    .extract({ left: 0, top: 0, width: 256, height: 256 })
    .resize({ height: Math.round(256 * (100 / 204) * VIEWPORT_FIT), fit: 'contain' })
    .png()
    .toBuffer({ resolveWithObject: true });
  const shelterScale = shelter.info.height / 256;
  await sharp(background)
    .composite([{
      input: shelter.data,
      left: Math.round(270 * VIEWPORT_FIT - shelter.info.width / 2),
      top: Math.round(480 * VIEWPORT_FIT - shelterAsset.anchorY * shelterScale),
    }])
    .png()
    .toFile(`${outputDir}/map-shelter-overlay.png`);
}

await Promise.all([renderAnimationHtml(), renderMapOverlay()]);
