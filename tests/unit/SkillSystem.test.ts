...
describe('SkillSystem timeline', () => {
  it('학습 직후 1초 쿨타임 뒤 명시적 요청으로만 시전한다', () => {
    ...
    expect(system.snapshot('aquaBeam')).toMatchObject({
      learned: true, ready: false, cooldownRemainingMs: 1000,
    });
    expect(system.step(1000, context)).toEqual([]);
    expect(system.snapshot('aquaBeam').ready).toBe(true);
    expect(system.requestCast('aquaBeam', 1000, context).status).toBe('started');
  });

  it('대상 없음은 쿨타임과 sequence를 소비하지 않는다', () => {
    const system = new SkillSystem();
    const empty = { player: { x: 0, y: 0 }, enemies: [] };
    system.learn('safetyReport', 0);
    system.step(1000, empty);
    const before = system.snapshot('safetyReport');

    expect(system.requestCast('safetyReport', 1000, empty)).toEqual({
      status: 'noTarget', events: [],
    });
    expect(system.snapshot('safetyReport')).toEqual(before);
  });
  ...
  it('ready 기술도 step에서 자동 시작하지 않고 요청한 기술만 시작한다', () => {
    const system = new SkillSystem();
    ...
    expect(startedIds(system.step(22_000, context))).toEqual([]);
    expect(startedIds(system.requestCast('aquaBeam', 22_000, context).events))
      .toEqual(['aquaBeam']);
    expect(startedIds(system.requestCast('tailSwipe', 22_000, context).events))
      .toEqual(['tailSwipe']);
    expect(startedIds(system.requestCast('safetyReport', 22_000, context).events))
      .toEqual(['safetyReport']);
  });

  it('readyAt이 달라도 자동 우선순위 없이 요청 상태를 정확히 반환한다', () => {
    const system = new SkillSystem();
    system.learn('safetyReport', 0);
    ...
    expect(system.step(30_000, context)).toEqual([]);
    expect(system.snapshot('safetyReport').ready).toBe(true);
    expect(system.snapshot('tailSwipe').ready).toBe(true);
    expect(system.requestCast('tailSwipe', 30_000, context).status).toBe('started');
    expect(system.requestCast('safetyReport', 30_000, context).status).toBe('started');
  });

  it('대상이 없는 ready는 보존하고 다른 기술의 성공과 독립적이다', () => {
    const system = new SkillSystem();
    ...
    expect(system.requestCast('tailSwipe', 10_000, {
      player: { x: 0, y: 0 },
      enemies: [farTarget],
    })).toEqual({ status: 'noTarget', events: [] });
    expect(startedIds(system.requestCast('aquaBeam', 10_000, {
      player: { x: 0, y: 0 },
      enemies: [farTarget],
    }).events)).toEqual(['aquaBeam']);
    expect(system.snapshot('tailSwipe')).toMatchObject({ ready: true, cooldownRemainingMs: 0 });
    expect(startedIds(system.requestCast('tailSwipe', 10_000, {
      player: { x: 1000, y: 0 },
      enemies: [farTarget],
    }).events)).toEqual(['tailSwipe']);
  });
  ...
  it('cooldown은 성공한 cast 시작 시점부터 다시 계산하고 active cast를 snapshot한다', () => {
    ...
    system.requestCast('tailSwipe', 8000, context);
  ...
  it('aqua는 현재 HP, boss, 위협도, 거리, 생성 순서 순으로 최초 표적을 고른다', () => {
    ...
    const events = system.requestCast('aquaBeam', 10_000, context).events;
  ...
  it('aqua 문맥 검증과 표적 선택은 scheduler 외 전체 정렬을 추가하지 않는다', () => {
    ...
    try {
      events = system.requestCast('aquaBeam', 10_000, {
        player: { x: 0, y: 0 },
        enemies: [
  ...
  it('aqua는 600ms 동안 한 번만 retarget하고 남은 charge를 보존한다', () => {
    const system = learnedSkillSystem('aquaBeam');
    system.requestCast('aquaBeam', 10_000, {
      player: { x: 0, y: 0 },
  ...
  it('safety는 300ms 뒤 start snapshot 중 생존한 모든 적만 동시에 impact한다', () => {
    ...
    system.requestCast('safetyReport', 22_000, {
      player: { x: 0, y: 0 }, enemies: initial,
    });
  ...
  it('동일 timestamp 재호출은 event를 중복 emit하지 않고 reset은 cast sequence까지 복구한다', () => {
    ...
    expect(system.requestCast('tailSwipe', 17_000, context).events.at(0))
      .toMatchObject({ castId: 'tailSwipe:1' });
  });
  ...
  it('서로 다른 deadline의 동시 pending cast를 큰 time leap에서도 impactAtMs 순으로 처리한다', () => {
    ...
    expect(startedIds(system.requestCast('safetyReport', 22_000, context).events))
      .toEqual(['safetyReport']);
    expect(startedIds(system.requestCast('aquaBeam', 22_200, context).events))
      .toEqual(['aquaBeam']);
  ...
  it('큰 timestamp를 결정적으로 처리하고 감소 timestamp와 invalid 입력은 state 변경 전에 거부한다', () => {
    ...
    expect(system.requestCast('aquaBeam', 1_000_000_010_000, context).events.at(0)).toMatchObject({
